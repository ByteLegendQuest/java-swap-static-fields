...
`git clone https://github.com/ByteLegendQuest/java-swap-static-fields`

A project directory named `java-swap-static-fields` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-swap-static-fields` directory which is created just now.
